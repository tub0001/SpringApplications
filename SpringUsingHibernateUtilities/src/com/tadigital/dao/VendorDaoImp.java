...
public class VendorDaoImp implements VendorDao{
	...
	public boolean loginProcess(Vendor v) {
		...
		query.setParameter("user_name", v.getVendor_name());
		query.setParameter("pwd", v.getVendor_password());
	...
	public boolean updateVendor(Vendor v) {
		...
		Session session = sessionFactory.openSession();
		Transaction transaction = session.beginTransaction();
		Vendor vendor = session.get(Vendor.class, v.getVendor_id());
		if(vendor != null) {
			vendor.setVendor_name((v.getVendor_name()));
			vendor.setVendor_email(v.getVendor_email());
